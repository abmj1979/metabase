  ...
  ([inner-query form source-table & sources]
   (mbql.u/replace form
     ...
     (let [{table-id :table_id, :as field} (qp.store/field id)
           {join-alias :alias}             (or (find-join-against-table inner-query table-id)
                                               #_(throw (ex-info (tru "Bad :field clause {0}: Field {1} is from a Table not in this query"
                                                                    (pr-str &match)
                                                                    (format "%s.%s"
                                                                            (:name (table table-id))
                                                                            (:name field)))
                                                               {:type    qp.error-type/invalid-query
                                                                ...
                                                                :sources (mapv (fn [source-table]
                                                                                 {:id   source-table
                                                                                  :name (some-> source-table table :name)})
                                                                               (cons source-table sources))
                                                                :path    &parents})))]
       (log/warn (str (u/colorize 'yellow (trs "Bad :field clause {0}: clause should have a :join-alias. Guessing join {1}"
                                               (pr-str &match)
                                               (pr-str join-alias)))
                      \newline
                      (trs "Path: {0}" (pr-str &parents))))
       (if join-alias
